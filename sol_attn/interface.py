...
def _sol_attn_cute(
    ...
    *,
    ...
    with torch.cuda.device(q.device):
        ...
        key = (q.device.index, arch, batch, tokens, heads, kv_splits, force_local_blocks)
        ...
        if arch == (9, 0):
            ...
            # SM120's H3 extension accepts an explicit route tensor. The
            # stock policy compiles with external_route=False and therefore
            # receives a one-byte placeholder that is never dereferenced.
...
def sol_attn(
    ...
    *,
    ...
    ``force_local_blocks`` forces query block ±1 exact (default True).
    Disabling it leaves these blocks subject to the normal threshold.
    ...
    if type(force_local_blocks) is not bool:
        raise TypeError("force_local_blocks must be bool")
