...
class SparkReweightForwardSm120:
    ...
    def kernel(
        ...
        q_tile_idx = cute.arch.make_warp_uniform(q_tile_idx + q_block_start)
        parent_idx = cutlass.Int32(mMap[q_tile_idx]) - parent_start
        summary_head = cute.arch.make_warp_uniform(head_idx)
        head_idx = cute.arch.make_warp_uniform(head_idx + head_start)
        ...
        mVC_slice = mVC[None, None, summary_head, summary_batch]
        mAK_slice = mAK[None, None, summary_head, summary_batch]
        ...
        gAK = cute.local_tile(mAK_slice, (N, D), coord=(None, 0))
        ...
        tAKsK, tAKgK = cute.nvgpu.cpasync.tma_partition(
            tma_atom_AK, *cta_coord_layout,
            cute.group_modes(sK, 0, 2), cute.group_modes(gAK, 0, 2),
        )
        ...
        for route_group in cutlass.range(
                ...
                for word in cutlass.range_constexpr(2):
                    ...
                    if valid:
                        ...
                        if cutlass.const_expr(self.external_route):
                            ...
                            exact = (col_mean > threshold) and valid
                            # A NaN cutoff marks a numerically unsafe Top-K
                            # boundary. Hybrid mode uses the explicit route for
                            # only that query-block/head and leaves every other
                            # row on the stock scalar-threshold selector.
                    ...
                    if cutlass.const_expr(self.export_route):
                        if valid:
                            mRouteMask[batch_idx, q_tile_idx, head_idx, group_start + off] = (
            ...
            has_approx = (exact_count < valid_blocks) and cutlass.const_expr(not self.exact_only)
            # Keep the route-score fragment separate: reusing tSrS for this
            # extra QK corrupts mixed threshold-mode results in CuTe lowering.
            tArS = cute.make_rmem_tensor_like(tSrS, self.acc_dtype)
            if has_approx:
            ...
            if has_approx:
                apply_route_mask(tArS, tScS, column_masks, q_len)
                row_scale = online_softmax(
                    tArS, max_m, sum_m, scale_softmax_log2e,
                )
                rescale_o_for_next_acc(tOrO, row_scale)
                tOrP_frg = cute.make_rmem_tensor_like(
                    tArS, self.K_dtype
                )
                tOrP_frg.store(tArS.load().to(self.K_dtype))
    ...
    def __call__(
        ...
        tma_atom_AK, tma_tensor_AK = cute.nvgpu.cpasync.make_tiled_tma_atom(
            g2s_op, ak_nkl, self.K_smem_layout, (N, D), num_multicast=1,
        )
        ...
        self.kernel(
            ...
            route_mask,
            lse_target,
            tma_tensor_AK, lm, mapping, q_block_start, parent_start, head_start,
            tma_atom_Q,
            tma_atom_K,
            tma_atom_V,
            tma_atom_AK,
...
def add_virtual_log_mass(scores: cute.Tensor, coords: cute.Tensor,
                         ...
                         parent: cutlass.Int32, head: cutlass.Int32,
                         group_start: cutlass.Int32, num_blocks: cutlass.Int32,
                         scale_log2e: cutlass.Float32):
    scores_mn = layout_utils.reshape_acc_to_mn(scores)
    coords_mn = layout_utils.reshape_acc_to_mn(coords)
    for m in cutlass.range_constexpr(cute.size(scores_mn, mode=[0])):
        for n in cutlass.range_constexpr(cute.size(scores_mn, mode=[1])):
            block = group_start + coords_mn[m, n][1]
            if block < num_blocks:
                # The production online softmax keeps its max in unscaled-dot
                # units; LM is in natural-log units. AV already is a mean, so
