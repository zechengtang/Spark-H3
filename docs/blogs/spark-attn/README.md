...
| Method | PSNR (dB) ↑ | SSIM ↑ | LPIPS ↓ | Attn<br>speedup ↑ | per-NFE<br>DiT speedup ↑ | density ↓ | Denoising<br>time (s, 19 NFE) ↓ |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
Following the NFE-normalized convention of [OpenVDN](https://openvdn.github.io/), all methods run the same 19 NFE (20 requested denoising steps), so the DiT speedup equals the per-NFE latency ratio: per-NFE latencies are 30.63 s for Dense, 19.21 s for Sol-H3, 17.98 s for Spark-H3-10pct and 19.91 s for Spark-H3-20pct. No step reduction is used; the speedups come from cheaper evaluations, not fewer steps. The first 4 of 19 steps and the first attention layer stay dense in all sparse methods, so per-step cost varies within each run, but the ratio of totals is identical to the ratio of mean per-NFE latencies.
