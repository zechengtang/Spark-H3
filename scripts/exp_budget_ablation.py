"""Exponential-decrease step budgets vs linear decrease: 5s/480p, GPUs 0-3.
...
4..18 sum to exactly 330 (largest-remainder rounding, min 1 block/head):

- `uniform`: 22 blocks/head everywhere (reused, verified).
- `step_13to7`: linear decrease 28.5->15.5 blocks/head (reused, verified).
- `step_exp_r95` / `step_exp_r93` / `step_exp_r90`: b_e proportional to
  rate^(e-4) for rate in {0.95, 0.93, 0.90}.
- `step_importance`: SAME-CASE ORACLE, not a deployable predictor: b_e
  proportional to max(measured_gain_e, 0.01), where the gains are the
  per-evaluation dense-gain curve measured on these same four cases in the
  dense_step_ablation experiment. It upper-bounds what any step-axis budget
  predictor of this proportional form could achieve here.
...
EXP = Path('/autodl-fs/data/h3_experiments/exp_budget_5s_480p_20260920')
OUT = Path('/autodl-fs/data/h3_outputs') / EXP.name
BASELINE = Path('/autodl-fs/data/h3_experiments/ramp_budget_5s_480p_20260920')
GAINS_SOURCE = Path('/autodl-fs/data/h3_experiments/dense_step_ablation_5s_480p_20260920')
REUSED_ARMS = ('uniform', 'step_13to7')
EXP_RATES = dict(step_exp_r95=0.95, step_exp_r93=0.93, step_exp_r90=0.90)
NEW_ARMS = tuple(EXP_RATES) + ('step_importance',)
...
def measured_gains():
    """Per-evaluation dense gains (dPSNR) from the dense-step experiment."""
    results = read(GAINS_SOURCE / 'results.json')
    gains = results['curve']['gains']
    return np.array([gains[str(e)] for e in SPARSE_EVALS])
...
def prepare():
    ...
    # Exponential and oracle budget tables.
    gains = measured_gains()
    budgets = {arm: proportional_budgets(rate ** np.arange(len(SPARSE_EVALS)))
               for arm, rate in EXP_RATES.items()}
    budgets['step_importance'] = proportional_budgets(np.maximum(gains, 0.01))
    ...
    write(EXP / 'protocol.json', dict(cases=cases, arms=['dense', *WORKER_ARMS],
        ...
        methods=dict(uniform='22 blocks/head at every sparse evaluation',
            ...
            step_exp_r95='b_e proportional to 0.95^(e-4), geometric mild',
            step_exp_r93='b_e proportional to 0.93^(e-4)',
            step_exp_r90='b_e proportional to 0.90^(e-4)',
            step_importance='SAME-CASE ORACLE (not deployable): b_e proportional to max(measured dense '
                'gain_e, 0.01); gains measured on these same four cases in dense_step_ablation'),
        oracle_gains_dpsnr={str(e): float(g) for e, g in zip(SPARSE_EVALS, gains)},
        oracle_gains_source=str(GAINS_SOURCE / 'results.json'),
...
def smoke(slot):
    """step_exp_r93 for case 3, end to end through scoring."""
    assert slot == 0 and CASES[0] == 3
    generate(slot, (0,), ('step_exp_r93',))
    full_arms = base.ARMS
    base.ARMS = ('dense', 'step_exp_r93')
    try:
        base.decode(0)
        base.score(0)
    finally:
...
def report():
    ...
    uniform, linear = by_arm['uniform'], by_arm['step_13to7']
    for s in summaries:
        s['delta_vs_uniform'] = {key: s[key] - uniform[key]
            for key in ('psnr_db', 'ssim', 'lpips', 'denoise_seconds')}
        s['delta_vs_step_13to7'] = {key: s[key] - linear[key]
            for key in ('psnr_db', 'ssim', 'lpips', 'denoise_seconds')}
    ...
    lines = ['# Exponential-decrease step budgets vs linear decrease: 5s 480p', '',
        ...
        'per-head budgets under the production Spark reweighting branch. `step_importance` is a '
        '**same-case oracle** (budgets proportional to dense gains measured on these same four prompts), '
        'not a deployable predictor. The uniform and step_13to7 baselines and the dense references are '
        'reused from the ramp-budget experiment after config and SHA256 verification.', '',
        '## Per-evaluation budgets (blocks/head)', '',
        '| Evaluation | uniform | step_13to7 | step_exp_r95 | step_exp_r93 | step_exp_r90 | step_importance |',
        '|---|---:|---:|---:|---:|---:|---:|']
    for e in SPARSE_EVALS:
        b = arm_budgets
        lines.append(f"| {e} | {b('uniform')[e]} | {b('step_13to7')[e]} | {b('step_exp_r95')[e]} | "
                     f"{b('step_exp_r93')[e]} | {b('step_exp_r90')[e]} | {b('step_importance')[e]} |")
    lines += ['', '## Averages over four prompts', '',
        '| Arm | PSNR | SSIM | LPIPS | Denoise seconds | dPSNR vs uniform | dLPIPS vs uniform | dPSNR vs linear | dLPIPS vs linear | PSNR wins | LPIPS wins |',
        '|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|']
    for s in summaries:
        du, dl = s['delta_vs_uniform'], s['delta_vs_step_13to7']
        w = wins[s['arm']]
        lines.append(f"| {s['arm']} | {s['psnr_db']:.4f} | {s['ssim']:.6f} | {s['lpips']:.6f} | "
                     f"{s['denoise_seconds']:.2f} | {du['psnr_db']:+.4f} | {du['lpips']:+.6f} | "
                     f"{dl['psnr_db']:+.4f} | {dl['lpips']:+.6f} | {w['psnr']}/4 | {w['lpips']}/4 |")
    ...
    lines += ['', '## Verdict', '',
        ...
        f"{best['delta_vs_step_13to7']['psnr_db']:+.4f} dB vs the linear ramp). "
        'Exponential shapes concentrate budget early while keeping a geometrically decaying tail; '
        'steeper rates approach the oracle shape but risk starving late evaluations (the LPIPS '
        'tail-starvation tradeoff seen with sigma^2 in the sigma-budget experiment). Compare the '
        'r95/r93/r90 means, per-case wins, and the oracle gap in the tables above to locate the decay '
        'rate sweet spot.', '',
        ...
        'same fixed budget; they do not test budget scaling. The oracle arm uses gains measured on the '
        'same four prompts, so it overstates what a causal predictor could achieve. Four prompts and one '
...
def launch(stage):
    ...
    while any(p.poll() is None for p in processes):
        write(EXP / 'status.json', dict(status='running', stage=stage, gpus=list(GPUS),
            ...
            expected_records=28, expected_quality=24))
        time.sleep(10)
...
if __name__ == '__main__':
    ...
    if args.stage == 'run':
        ...
        launch('finish')
        report()
        write(EXP / 'status.json', dict(status='complete', new_generations=16, quality=24))
