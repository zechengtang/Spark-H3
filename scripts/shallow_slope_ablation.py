"""Shallow-slope completion of the step-budget slope scan: 5s/480p, GPU 0.

The slope scan (slope_budget_ablation.py) covered d = 0 (uniform) and d = 6.5
(step_13to7) with nothing in between, plus d = 8..20. This experiment fills the
shallow side with d in {1.5, 3, 4.5}: symmetric linear ramps 22+-d blocks/head
across sparse evaluations 4..18 (endpoints 23.5->20.5, 25->19, 26.5->17.5),
largest-remainder rounding, min 1 block/head, exact per-head total 330 and
grand total 905,520 routed blocks per generation.

- `uniform` (d=0) and `step_13to7` (d=6.5): reused from the ramp-budget
  experiment after config and SHA256 verification, exactly as in the slope scan.
- `step_lin_d1p5`, `step_lin_d3`, `step_lin_d4p5`: new arms.

Stages: prepare/smoke/worker/finish/report/run.
"""
...
EXP = Path('/autodl-fs/data/h3_experiments/shallow_slope_5s_480p_20260920')
OUT = Path('/autodl-fs/data/h3_outputs') / EXP.name
BASELINE = Path('/autodl-fs/data/h3_experiments/ramp_budget_5s_480p_20260920')
SLOPE_EXP = Path('/autodl-fs/data/h3_experiments/slope_budget_5s_480p_20260920')
REUSED_ARMS = ('uniform', 'step_13to7')
SLOPES = dict(step_lin_d1p5=1.5, step_lin_d3=3, step_lin_d4p5=4.5)
...
def prepare():
    ...
    for arm in REUSED_ARMS:
        ...
        for case in CASES:
            ...
            try:
                ...
                if got_config.get('sol_extra_dense_layers') is None:  # older serialization of ()
                    got_config['sol_extra_dense_layers'] = []
                checks = [
    ...
    write(EXP / 'protocol.json', dict(cases=cases, arms=['dense', *WORKER_ARMS],
        ...
        methods=dict(uniform='22 blocks/head at every sparse evaluation (d=0)',
            ...
            step_lin_d1p5='Linear ramp 23.5->20.5 (d=1.5)',
            step_lin_d3='Linear ramp 25->19 (d=3)',
            step_lin_d4p5='Linear ramp 26.5->17.5 (d=4.5)'),
...
def smoke(slot):
    """step_lin_d3 for case 3, end to end through scoring."""
    assert slot == 0 and CASES[0] == 3
    generate(slot, (0,), ('step_lin_d3',))
    full_arms = base.ARMS
    base.ARMS = ('dense', 'step_lin_d3')
    try:
        base.decode(0)
        base.score(0)
    finally:
...
def report():
    ...
    lines = ['# Shallow-slope completion of the step-budget slope scan: 5s 480p', '',
        'Four prompts, seed 42, 120 frames at 24 fps, 832x480, 20 requested steps (19 evaluations). '
        'GPU 0. First four evaluations and layer 0 remain dense. Every arm spends exactly the uniform '
        ...
        '4->18. This experiment fills the shallow side of the slope scan (d in {1.5, 3, 4.5}) between '
        'uniform (d=0) and step_13to7 (d=6.5). The uniform and step_13to7 baselines and the dense '
        ...
        '| Evaluation | uniform | step_13to7 | d1p5 | d3 | d4p5 |',
        '|---|---:|---:|---:|---:|---:|']
    for e in SPARSE_EVALS:
        b = arm_budgets
        lines.append(f"| {e} | {b('uniform')[e]} | {b('step_13to7')[e]} | {b('step_lin_d1p5')[e]} | "
                     f"{b('step_lin_d3')[e]} | {b('step_lin_d4p5')[e]} |")
    ...
    lines += ['', '## Combined slope response (with the steep-side slope scan)', '']
    slope_file = SLOPE_EXP / 'results.json'
    combined = list(response)
    if slope_file.is_file():
        for row in read(slope_file)['slope_response']:
            if row['arm'] not in REUSED_ARMS:
                combined.append((row['d'], row['arm'], row['psnr_db'], row['lpips'], row['ssim']))
        combined.sort()
        lines += ['| d (blocks/head) | Arm | Mean PSNR | Mean SSIM | Mean LPIPS |', '|---|---|---:|---:|---:|']
        for d, a, p, l, s in combined:
            lines.append(f'| {d:g} | {a} | {p:.4f} | {s:.6f} | {l:.6f} |')
        peak_d, peak_arm, peak_p, _, _ = max(combined, key=lambda x: x[2])
        bestl_d, bestl_arm, _, bestl_l, _ = min(combined, key=lambda x: x[3])
        lines += ['',
            f'Combined over d in {{0, 1.5, 3, 4.5, 6.5, 8, 11, 14, 17, 20}}: PSNR peaks at d={peak_d:g} '
            f'(`{peak_arm}`, {peak_p:.4f} dB); best mean LPIPS at d={bestl_d:g} (`{bestl_arm}`, {bestl_l:.6f}).']
    else:
        lines.append('Slope-scan results.json was not available at report time.')
...
def launch(stage):
    ...
    while any(p.poll() is None for p in processes):
        write(EXP / 'status.json', dict(status='running', stage=stage, gpus=list(GPUS),
            ...
            expected_records=24, expected_quality=20))
        time.sleep(10)
...
if __name__ == '__main__':
    ...
    if args.stage == 'run':
        ...
        launch('finish')
        report()
        write(EXP / 'status.json', dict(status='complete', new_generations=12, quality=20))
